...
int main(int argc, char *argv[])
{
	...
	for(i = 0x0; i < 0x1E00; i++) {
		king_kram_write(i ^ (i << 4) ^ (i >> 4));
	}
	king_set_kram_write(0, 1);
	printstr("Cellophane Test!", 8, 0x20, 1);

	tetsu_set_cellophane_coeffs(1, 1, 1, 1, 7, 7, 7);
	tetsu_set_cellophane_color(0x8088);
	tetsu_set_cellophane_control(1, 1, TETSU_NO_COEFF, TETSU_NO_COEFF, TETSU_COEFF_1,
					TETSU_NO_COEFF, TETSU_NO_COEFF, TETSU_NO_COEFF, TETSU_NO_COEFF);
	for(;;) {
		for(i = 0; i < 8; i++) {
			tetsu_set_cellophane_coeffs(1, i, i, i, 7-i, 7-i, 7-i);
			delay();
		}
		for(i = 0; i < 8; i++) {
			tetsu_set_cellophane_coeffs(1, 7-i, 7-i, 7-i, i, i, i);
			delay();
		}
	}
	return 0;
