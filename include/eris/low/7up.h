...
#ifndef _LIBPCFX_LOW_7UP_H_
#define _LIBPCFX_LOW_7UP_H_
 ...
 */
void low_sup_setreg(int chip, int reg, int value);
 ...
 *
 * \sa low_sup_get_video_mode()
 */
void low_sup_set_video_mode(int chip, int hdispstrt, int hsyncwid,
 ...
 *
 * \sa low_sup_set_video_mode()
 */
void low_sup_get_video_mode(int chip, int *hdispstrt, int *hsyncwid,
				int *hdispend, int *hdispwid, int *vdispstrt,
				int *vsyncwid, int *vdispwid, int *vdispend);
 ...
 */
void low_sup_set_vram_write(int chip, u16 addr);
/*! \brief Write to VRAM on a 7up chip.
 *
 * \param chip Which 7up to write data to. (0 ~ 1)
 * \param data Data to write.
 */
void low_sup_vram_write(int chip, u16 data);
 ...
 */
void low_sup_set_vram_read(int chip, u16 addr);
 ...
 */
u16 low_sup_vram_read(int chip);
 ...
 *
 * \sa low_sup_get_control()
 */
void low_sup_set_control(int chip, int increment, int bg_show,
				int spr_show);
 ...
 * \param spr_show Whether sprites should show or not.
 *
 * \sa low_sup_set_control()
 */
void low_sup_get_control(int chip, int *increment, int *bg_show,
				int *spr_show);
 ...
 *                      sprite 0.
 */
void low_sup_set_interrupts(int chip, int vblank_irq, int raster_irq,
 ...
 */
void low_sup_set_interrupt_raster(int chip, int raster);
 ...
 *
 * \sa low_sup_get_scroll()
 */
void low_sup_set_scroll(int chip, u16 x, u16 y);
 ...
 * \param y Top-left Y coordinate of the background.
 *
 * \sa low_sup_set_scroll()
 */
void low_sup_get_scroll(int chip, u16 *x, u16 *y);
 ...
 */
void low_sup_do_dma(int chip, u16 src, u16 dst, u16 len);
 ...
 */
void low_sup_setup_dma(int chip, int satb_repeat, int src_dec,
 ...
 */
void low_sup_set_satb_address(int chip, u16 addr);
 ...
 *
 * \sa low_sup_get_access_width()
 */
void low_sup_set_access_width(int chip, int cg_mode,
 ...
 * \param vram_px_w VRAM pixel width. See hardware documentation. (0 ~ 3)
 *
 * \sa low_sup_set_access_width()
 */
void low_sup_get_access_width(int chip, int *cg_mode,
				sup_low_mapsize *mapsize, int *spr_px_w,
				int *vram_px_w);
